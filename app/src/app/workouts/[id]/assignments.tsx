...
export default function WorkoutAssignmentsScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Process Assignments
      const allAssigned: { id: string; full_name: string; email: string }[] = [];
      ...
      if (assignmentsData) {
        assignmentsData.forEach(
          (item: {
            ...
            const studentArray = Array.isArray(item.student)
              ? item.student
              : item.student
                ? [item.student]
                : [];
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: Initialization hook
  useEffect(() => {
    if (id && user?.id) {
      fetchData();
    }
  ...
  const handleUpdateAssignments = async (selectedIds: string[]) => {
    ...
    try {
      ...
      if (toAdd.length > 0) {
        ...
        const { error: addError } = await supabase.from('workout_assignments').insert(assignments);
      ...
      Alert.alert(
        'Erro',
        `Erro ao atualizar atribuições: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  };

  const handleRemoveStudent = (studentId: string) => {
    Alert.alert('Remover Aluno', 'Deseja remover este aluno do treino?', [
    ...
    <View style={{ flex: 1, backgroundColor: '#09090B' }}>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        >
          <TouchableOpacity
            onPress={() => router.back()}
            style={{ backgroundColor: '#18181B', padding: 10, borderRadius: 12, marginRight: 16 }}
          >
