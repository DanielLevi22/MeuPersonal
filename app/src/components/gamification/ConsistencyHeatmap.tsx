  ...
  for (let w = 0; w < numWeeks; w++) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      if (goal) {
        ...
        const workoutMet =
          (goal.workout_target || 0) === 0 || (goal.workout_completed || 0) >= goal.workout_target;

        if (mealsMet && workoutMet)
          level = 2; // Perfect
    ...
    <View className="mb-8">
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View className="flex-row gap-[3px]">
          {weeks.map((week) => (
            <View key={week[0].date} className="gap-[3px]">
                  ...
                  className={`w-3 h-3 rounded-[2px] ${
                    day.isFuture
                      ...
                      : day.level === 2
                        ? 'bg-emerald-500'
                        : day.level === 1
                          ? 'bg-emerald-900 border border-emerald-500/30'
                          : 'bg-zinc-800'
                  }`}
                />
              ))}
