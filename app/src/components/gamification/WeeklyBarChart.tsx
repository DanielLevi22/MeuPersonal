...
export function WeeklyBarChart({ data }: WeeklyBarChartProps) {
  ...
  const weekData = useMemo(() => {
    ...
    return data.map((d) => {
      ...
      return {
        ...
        workoutProgress:
          d.workout_target > 0 ? Math.min(d.workout_completed / d.workout_target, 1) : 0,
      };
    });
  }, [data]);
  ...
  if (!data || data.length === 0) {
    return (
      <View
        className="bg-zinc-900/50 rounded-3xl p-6 items-center justify-center border border-zinc-800"
        style={{ height: 200 }}
      >
      ...
      <View className="flex-row justify-between items-center mb-6 h-10">
        {selectedData ? (
          <Animated.View
            ...
            className="flex-1 flex-row items-center justify-between bg-zinc-800/80 px-3 py-2 rounded-xl"
          >
        ...
        {weekData.map((day, index) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              style={{ width: (screenWidth - 48 - 48) / 7 }}
            >
              <Animated.View
              ...
              >
                ...
                {/* Label */}
                <Text
                  className={`text-[10px] font-bold mt-2 ${isSelected ? 'text-white' : 'text-zinc-500'}`}
                >
                  {day.label}
                </Text>
              </Animated.View>
