    ...
    <View className="mb-8">
      ...
      >
        {days.map((date, _index) => {
          ...
          const workoutDone =
            (goal?.workout_target || 0) === 0 ||
            (goal?.workout_completed || 0) >= (goal?.workout_target ?? 0);
          ...
          return (
            <View key={date} className="items-center gap-2">
              <Text
                className={`text-[10px] font-black uppercase ${isToday ? 'text-white' : 'text-zinc-600'}`}
              >
                {dayLabel}
              </Text>
