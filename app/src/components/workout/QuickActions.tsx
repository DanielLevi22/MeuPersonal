...
export function QuickActions({
  ...
  return (
    <View className="flex-row justify-between px-4 py-2">
      {actions.map((action) => {
        const isAiAction = action.id === 'ai-prescricao';
        return (
          <TouchableOpacity
            key={action.id}
          ...
          >
            ...
            <View className="mb-2">
              <View
                ...
                style={{
                  backgroundColor: isAiAction
                    ? 'rgba(157, 78, 221, 0.1)'
                    : 'rgba(0, 217, 255, 0.1)', // Subtle tint
                  borderColor: isAiAction ? 'rgba(157, 78, 221, 0.3)' : 'rgba(0, 217, 255, 0.3)',
                }}
              >
                <Ionicons
                  name={action.icon as keyof typeof Ionicons.glyphMap}
                  size={26}
                  color={isAiAction ? '#C084FC' : '#00D9FF'}
                />
              </View>
            </View>
