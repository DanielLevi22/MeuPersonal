...
export function useHealthData() {
      ...
      try {
        ...
        if (!isInitialized) {
          if (__DEV__) {
            ...
            setData((prev) => ({
              ...prev,
              error: 'Health Connect not initialized',
              loading: false,
            }));
          }
          return;
        ...
        // IMPORTANTE: Não chamar requestPermission() aqui automaticamente.
        // O ActivityResultLauncher do Health Connect precisa estar completamente
        // inicializado antes de ser invocado. Chamar no mount/useEffect causa
        // UninitializedPropertyAccessException. A permissão deve ser solicitada
        // apenas por ação direta do usuário (ex: botão na tela de onboarding).
        const granted = await getGrantedPermissions();
        const hasSteps = granted.some((p) => p.recordType === 'Steps' && p.accessType === 'read');
        const hasCalories = granted.some(
          (p) => p.recordType === 'ActiveCaloriesBurned' && p.accessType === 'read'
        );
  ...
  const fetchIOSData = useCallback(() => {
    const options = {
