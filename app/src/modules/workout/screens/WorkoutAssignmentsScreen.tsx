...
export default function WorkoutAssignmentsScreen() {
      ...
      if (assignmentsData) {
        assignmentsData.forEach(
          (item: {
            ...
            if (studentObj && !seenIds.has(studentObj.id)) {
              allAssigned.push(studentObj);
              seenIds.add(studentObj.id);
            }
          }
        );
      }
  ...
  const handleUpdateAssignments = async (selectedIds: string[]) => {
    ...
    try {
      ...
      Alert.alert('Erro', `Erro ao atualizar atribuições: ${(e as Error).message}`);
    }
  };
