...
import { ActivityIndicator, FlatList, Linking, Modal, RefreshControl, Text, TouchableOpacity, View } from 'react-native';
...
export default function LeaderboardScreen() {
  ...
  const [filterScope, setFilterScope] = useState<'global' | 'my_students'>(accountType === 'professional' ? 'my_students' : 'global');
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      if (accountType === 'professional' && user?.id && filterScope === 'my_students') {
        ...
        const students = linkedData.map(d => d.student as any).filter(Boolean); // Extract profiles
        const studentIds = students.map((s: any) => s.id);

        // 2. Fetch scores for these students
        // For monthly, we might get multiple rows per student (one per week start date possibly, or if granulariy is daily). 
        // Assuming ranking_scores is weekly granularity based on 'week_start_date'.
        // If monthly, we sum all rows where week_start_date >= month_start.
        ...
        // 3. Aggregate scores (Sum points if multiple rows per student)
        ...
        let mergedData = students.map((student: any) => ({
        ...
        return; // Done for this scope
      }

      // STRATEGY 2: Global Scope
      // THIS IS TRICKIER for Monthly: We need to sum scores for everyone, then sort top 50.
      // JS Aggregation for global might be heavy if many rows. 
      // Ideally DB should do it: .rpc() call would be best.
      // For now, let's keep fetching raw scores and aggregating in JS (assuming simple volume).
      
      const { data: scores, error: scoresError } = await supabase
        .from('ranking_scores')
        .select('student_id, points')
        .gte('week_start_date', startDate);
        // .order('points', { ascending: false }) // Can't order by raw points if we need to sum first
  ...
  const togglePeriod = (period: 'weekly' | 'monthly') => {
  ...
  const rest = entries.slice(3); // Although rest is slices, we are showing ALL entries in the list now as per previous request
      ...
      <View className="flex-1 pt-4"> 
        ...
        <View className="px-6 pb-6 items-center z-10 relative">
          ...
          <View className="absolute right-6 top-2 flex-row gap-3">
              <TouchableOpacity onPress={() => setShowFilterModal(true)} className="bg-zinc-900/80 border border-zinc-800 p-2.5 rounded-full">
                  <Ionicons name="filter" size={18} color={brandColors.primary.start} />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleSharePodium} className="bg-zinc-900/80 border border-zinc-800 p-2.5 rounded-full">
          ...
          {accountType === 'professional' && (
              <View className="flex-row bg-black p-1.5 rounded-2xl border border-zinc-800 mt-6 min-w-[280px]">
                  ...
                  >
                      <Text className={`text-xs font-black uppercase tracking-widest ${filterScope === 'my_students' ? 'text-white' : 'text-zinc-500'}`}>
                          Meus Alunos
                      </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                      <Text className={`text-xs font-black uppercase tracking-widest ${filterScope === 'global' ? 'text-white' : 'text-zinc-500'}`}>
                          Global
                      </Text>
                  </TouchableOpacity>
              </View>
            ...
            >
                <View className="bg-zinc-950 rounded-t-[32px] p-6 border-t border-zinc-800 shadow-2xl shadow-black">
                    ...
                    <View className="gap-3">
                        <TouchableOpacity 
                            onPress={() => { setFilterPeriod('weekly'); setShowFilterModal(false); }}
                        ...
                        >
                             <View className={`w-12 h-12 rounded-full items-center justify-center mr-4 ${
                                 ...
                                 <Ionicons name="calendar" size={24} color={filterPeriod === 'weekly' ? '#f97316' : '#71717a'} />
                             </View>
                             <View className="flex-1">
                                 <Text className={`text-lg font-bold ${filterPeriod === 'weekly' ? 'text-white' : 'text-zinc-300'}`}>
                                     Esta Semana
                                 </Text>
                        ...
                        <TouchableOpacity 
                            onPress={() => { setFilterPeriod('monthly'); setShowFilterModal(false); }}
                            className={`flex-row items-center p-4 rounded-2xl border ${
                        ...
                        >
                             <View className={`w-12 h-12 rounded-full items-center justify-center mr-4 ${
                                 ...
                                 <Ionicons name="calendar-number" size={24} color={filterPeriod === 'monthly' ? '#f97316' : '#71717a'} />
                             </View>
                             <View className="flex-1">
                                 <Text className={`text-lg font-bold ${filterPeriod === 'monthly' ? 'text-white' : 'text-zinc-300'}`}>
                                     Este Mês
                                 </Text>
                        ...
                        >
                             <View className={`w-12 h-12 rounded-full items-center justify-center mr-4 ${
                                 ...
                                 <Ionicons name="options" size={24} color={filterPeriod === 'custom' ? '#f97316' : '#71717a'} />
                             </View>
                             <View className="flex-1">
                                 <Text className={`text-lg font-bold ${filterPeriod === 'custom' ? 'text-white' : 'text-zinc-300'}`}>
                                     Personalizado
                                 </Text>
                    ...
                    <TouchableOpacity 
                        ...
                        onPress={() => setShowFilterModal(false)}
                    >
                         <Text className="text-zinc-500 font-bold text-center uppercase tracking-widest">Cancelar</Text>
                    </TouchableOpacity>
                </View>
            </TouchableOpacity>
        </Modal>
                ...
                >
                    <View className="w-24 h-24 rounded-full border-4 border-yellow-500 mb-4 items-center justify-center bg-zinc-800">
                         {selectedStudent?.avatar_url ? ( // Assuming Image usage or text fallback logic here same as others
                             ...
                             <Text className="text-white text-3xl font-bold">{selectedStudent?.name.charAt(0)}</Text>
                         )}
                    </View>
