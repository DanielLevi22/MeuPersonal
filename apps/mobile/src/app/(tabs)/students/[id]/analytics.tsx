...
export default function StudentAnalyticsScreen() {
    ...
    const [overviewHistory, setOverviewHistory] = useState<any[]>([]);
    ...
    useFocusEffect(
        useCallback(() => {
            if (activeTab === 'overview' && studentId) {
                ...
                gamificationService.getWeeklyGoals(startDate, endDate, studentId)
        ...
        <ScreenLayout>
            <View className="flex-1">
                ...
                <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 0 }}>
                     ...
                     {activeTab === 'workouts' && (
                        <WorkoutAnalytics studentId={studentId} />
                     )}

                     {activeTab === 'nutrition' && (
                        <StudentNutritionAnalytics studentId={studentId} />
                     )}
                </ScrollView>
            </View>
        </ScreenLayout>
...
function TabButton({ label, isActive, onPress }: { label: string, isActive: boolean, onPress: () => void }) {
    return (
        ...
        >
            <Text className={`font-bold ${isActive ? 'text-white' : 'text-zinc-500'}`}>
                {label}
            </Text>
        </TouchableOpacity>
    );
