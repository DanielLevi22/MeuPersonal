...
const MuscleOverlay = ({ position, intensity }: { position: any, intensity: number }) => {
...
export function BodyHeatmap2D({ intensities = {
    chest: 0.8,
    back: 0.2,
    shoulders: 0.5,
    arms: 0.4,
  ...
  return (
    <View className="flex-1 items-center justify-center relative">
        ...
        >
          <View className="relative w-full h-full items-center justify-center">
            ...
            <View style={{ position: 'absolute', width: '80%', height: '90%' }}>
              {view === 'front' ? (
                <>
                  <MuscleOverlay position={MUSCLE_OVERLAYS.front.chest} intensity={intensities.chest || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.front.shoulders} intensity={intensities.shoulders || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.front.abs} intensity={intensities.abs || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.front.arms} intensity={intensities.arms || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.front.legs} intensity={intensities.legs || 0} />
                </>
              ) : (
                <>
                  <MuscleOverlay position={MUSCLE_OVERLAYS.back.back} intensity={intensities.back || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.back.shoulders} intensity={intensities.shoulders || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.back.arms} intensity={intensities.arms || 0} />
                  <MuscleOverlay position={MUSCLE_OVERLAYS.back.legs} intensity={intensities.legs || 0} />
                </>
              )}
            </View>
          </View>
      ...
      <View className="absolute top-4 bg-zinc-900/80 px-4 py-2 rounded-full border border-zinc-700">
        <Text className="text-zinc-300 text-xs">Pinça para zoom • Gire para rotacionar • Toque duplo para resetar</Text>
      </View>
      ...
      <View className="absolute bottom-4 flex-row gap-2">
        ...
        >
          <Text className={`font-bold ${view === 'front' ? 'text-white' : 'text-zinc-400'}`}>Frente</Text>
        </TouchableOpacity>
        <TouchableOpacity 
        ...
        >
          <Text className={`font-bold ${view === 'back' ? 'text-white' : 'text-zinc-400'}`}>Costas</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
