...
export function Podium({ topThree }: PodiumProps) {
  const getPodiumStyle = (rank: number) => {
    switch (rank) {
      ...
      case 1: return { height: 160, color: ['#FBBF24', '#B45309'], scale: 1.1 };
      // Silver (Metallic Blue-Gray)
      case 2: return { height: 130, color: ['#E2E8F0', '#475569'], scale: 1 };
      // Bronze (Deep Orange/Rust)
      case 3: return { height: 110, color: ['#FB923C', '#7C2D12'], scale: 0.9 };
...
function PodiumItem({ student, rank, style }: { student: any, rank: number, style: any }) {
    if (!student) {
        // Placeholder for empty slot
        return (
            <View className="items-center w-24 opacity-30" style={{ transform: [{ scale: style.scale }] }}>
    ...
    return (
        <View className="items-center w-24 z-10" style={{ transform: [{ scale: style.scale }] }}>
            <View className="mb-3 relative shadow-xl shadow-black">
              ...
              {student.avatar_url ? (
                ...
                <View 
                    ...
                    style={{ backgroundColor: brandColors.background.elevated, borderColor: style.color[0] }}
                >
            ...
            <Text className="text-[10px] font-black mb-2 uppercase tracking-wide" style={{ color: style.color[0] }}>
                {student.points} PTS
            </Text>
