        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                const completed = data.filter(d => 
                    (d.meals_completed || 0) >= (d.meals_target || 1)
                ).length;
        ...
        <Animated.View entering={FadeInDown.delay(300).springify()} className="gap-6 pb-20">
             ...
             <View className="flex-row gap-4">
                <View className="flex-1 bg-zinc-900 p-4 rounded-2xl border border-zinc-800 items-center">
                    <Text className="text-zinc-500 text-xs font-bold mb-1 uppercase tracking-wider">Aderência (28d)</Text>
                    <Text className={`text-2xl font-black ${stats.adherence >= 80 ? 'text-emerald-400' : stats.adherence >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                        {stats.adherence.toFixed(0)}%
                    </Text>
                </View>
                <View className="flex-1 bg-zinc-900 p-4 rounded-2xl border border-zinc-800 items-center">
                    <Text className="text-zinc-500 text-xs font-bold mb-1 uppercase tracking-wider">Dias na Meta</Text>
                    <Text className="text-white text-2xl font-black">
                        {stats.completedDays}<Text className="text-zinc-600 text-base">/{stats.totalDays}</Text>
                    </Text>
                </View>
             </View>
