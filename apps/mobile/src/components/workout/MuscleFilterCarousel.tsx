...
export function MuscleFilterCarousel({ selectedMuscle, onSelectMuscle, containerStyle }: MuscleFilterCarouselProps) {
  return (
    <View style={containerStyle}>
      ...
      >
        ...
        >
          <Ionicons name="grid-outline" size={18} color={!selectedMuscle ? '#FF6B35' : '#71717A'} style={{marginRight: 8}} />
          <Text className={`font-bold ${!selectedMuscle ? 'text-white' : 'text-zinc-500'}`}>Todos</Text>
        </TouchableOpacity>
        ...
        {MUSCLE_FILTERS.map((m) => (
          ...
          >
            <Ionicons name={m.icon as any} size={18} color={selectedMuscle === m.name ? '#FF6B35' : '#71717A'} style={{marginRight: 8}} />
            <Text className={`font-bold ${selectedMuscle === m.name ? 'text-white' : 'text-zinc-500'}`}>{m.name}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    </View>
