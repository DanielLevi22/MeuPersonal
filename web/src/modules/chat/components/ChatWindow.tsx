...
export function ChatWindow({ conversationId }: ChatWindowProps) {
  ...
  const receiverId =
    conversation?.personal_id === user?.id ? conversation?.student_id : conversation?.personal_id;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-4 p-5 border-b border-white/10 bg-gradient-to-r from-surface to-surface-highlight">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">{conversation.other_user?.email}</p>
        </div>
            ...
            {conversationMessages.map((message: ChatMessage, index: number) => {
              ...
              const showTime =
                ...
                new Date(message.created_at).getTime() -
                  new Date(conversationMessages[index - 1].created_at).getTime() >
                  60000;
              ...
              return (
                <div key={message.id}>
                  ...
                  <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    ...
                    >
                      <p
                        className={`text-sm leading-relaxed ${isMe ? "text-black" : "text-foreground"}`}
                      >
                        {message.content}
                      </p>
                      <div
                      ...
                      >
                        <span
                          className={`text-xs ${isMe ? "text-black/60" : "text-muted-foreground"}`}
                        >
                        ...
                        {isMe && (
                          <svg
                            className={`w-3 h-3 ${message.read_at ? "text-black" : "text-black/40"}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
      ...
      <form onSubmit={handleSend} className="p-5 border-t border-white/10 bg-surface">
        ...
        <p className="text-xs text-muted-foreground mt-2">{messageText.length}/1000 caracteres</p>
      </form>
    </div>
  );
}
