...
import type { Periodization } from "@/shared/hooks/usePeriodizations";
...
export function PeriodizationCard({
  ...
  return (
    <div className="relative bg-surface border border-white/10 rounded-xl p-6 hover:scale-[1.02] transition-all duration-300 shadow-lg hover:shadow-black/20 group">
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            <span
              className={`px-3 py-1 rounded-full text-xs font-bold border ${statusColors[periodization.status]}`}
            >
              {statusLabels[periodization.status]}
            </span>
            <span
              className={`px-3 py-1 rounded-full text-xs font-bold border bg-background/50 ${objectiveColors[periodization.objective]}`}
            >
          ...
          {periodization.student && (
            <p className="text-sm text-muted-foreground">👤 {periodization.student.full_name}</p>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-background/50 border border-white/5 rounded-lg p-3 text-center">
          ...
          <p className="text-lg font-bold text-foreground">
            {periodization.training_plans_count || 0}
          </p>
        </div>
        <div className="bg-background/50 border border-white/5 rounded-lg p-3 text-center">
      ...
      {periodization.notes && (
        <div className="mb-4">
          <p className="text-sm text-muted-foreground line-clamp-2">📝 {periodization.notes}</p>
        </div>
      )}
